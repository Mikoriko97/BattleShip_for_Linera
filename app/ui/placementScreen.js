...
function placementScreen(playerBoard, ships, options = {}) {
  ...
  function render() {
    ...
    const primaryButtonText = options.primaryButtonText || "START GAME";
    ...
    startGameBtn.textContent = primaryButtonText;
...
`
    ...
    headerBar.classList.add("lobby_bar");
    ...
    if (options.headerHtml) {
      headerBar.innerHTML = options.headerHtml;
    }
  ...
  function startGame() {
    ...
    if (typeof options.onSubmitBoard === "function") {
      submitting = true;
      startGameBtn.setAttribute("disabled", "");
      Promise.resolve(options.onSubmitBoard()).finally(() => {
        submitting = false;
      });
      return;
    }
